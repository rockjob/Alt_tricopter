...
// Return a pressure reading.
{
	...
	*/
	...
	//Serial.println("switch delay");
//	Serial.println(commanded_precision);
	//delay(67);
	switch(commanded_precision){
		case MODE_LOW:
		{	
		//Serial.println("low");
			delay(5); //  Wait 5ms for conversion to complete
			break;
		}
		case MODE_STANDARD:
		{
		//	Serial.println("std");
			delay(11); //  Wait 11 ms for conversion to complete
			break;
		}
		case MODE_HIGH:{
		//	Serial.println("high");
			delay(19); //  Wait 19 ms for conversion to complete
			break;
		}
		case MODE_ULTRA:{
		//	Serial.println("ultra");
			delay(67); //  Wait 67 ms for conversion to complete
			break;
		}
	};
...
// This can be modified to work outside of Arduino based MCU's  
{
	...
	*/
		...
		*/
		I2c.read(T5403_I2C_ADDR,location,1,&byteLow);
		I2c.read(T5403_I2C_ADDR,1,&byteHigh);
...
// This can be modified to work outside of Arduino based MCU's
{
	...
	// If i2c is selected for communication use i2c commands
		...
		I2c.write(T5403_I2C_ADDR, location, command);
	}
}